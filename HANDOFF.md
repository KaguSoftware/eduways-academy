...
- All translatable DB **content** is JSONB `{fa, en}` (`I18nText`); render with `tx(value, locale)`. `tx()` is for content only — never for UI chrome. Numbers via `formatNumber` (Persian digits in fa); money via `createMoney(t, locale)` / `useMoney()` (`src/lib/money.ts`, affixes in `common.money`/`common.moneyRange`). Dates via `formatDate` (Jalali in fa).
- **Every** user-facing string lives in `src/messages/{fa,en}.json` (namespaces: meta, nav, common, home, universities, programs, rankings, districts, services, calculator, stories, blog, about, contact, consultation, faq, footer, legal, errors, form, admin) — including `aria-label`s, placeholders, WhatsApp prefills, error toasts and legal copy. No bilingual literals or `locale === "fa" ? … : …` copy in `.ts`/`.tsx`. `npm run i18n:check` enforces fa/en key parity and that every admin label resolves.
...
- Admin tables are spec-driven: `src/lib/admin/specs.ts` holds **structure only** (column keys, types, sections) — all copy resolves through `adminText(t, table)` (`src/lib/admin/labels.ts`) against `admin.tables/fields/tableFields/fieldHelp/tableFieldHelp/options/sections`, each with a per-table → shared → `common.*` fallback so a repeated column is translated once. Writes go through server actions in `src/lib/admin/actions.ts` (service role, staff check, `revalidatePath("/", "layout")`); they return an **error code**, which `record-form.tsx` translates.
