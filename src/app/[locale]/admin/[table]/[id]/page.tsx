...
export default async function AdminRecordPage({ params }: { params: Promise<{ locale: string; table: string; id: string }> }) {
  ...
  const isNew = id === "new";
  const row = isNew ? Object.fromEntries(spec.fields.map((f) => [f.key, f.type === "boolean" ? false : f.key === "status" ? "published" : null])) : await getRow(table, id);
  if (!row) notFound();
  ...
  return (
    <div className="space-y-6">
      ...
      <RecordForm table={table} spec={spec} initial={row} isNew={isNew} canDelete={staff.role === "admin"} readOnly={!hasSupabase} />
    </div>
  );
}
