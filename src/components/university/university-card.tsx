 ...
 */
const opticalScale = (aspect: number) =>
    Math.min(1, Math.sqrt(1.5 / Math.max(aspect, 1.5)));
...
export function UniversityLogo({
    ...
    if (logo) {
        ...
        const pad = plate
            ? 0
            : Math.round(
                  size * (fit === "auto" ? 0.1 : shape === "wide" ? 0.03 : 0.08)
              );
        const width = square
            ? size
            : Math.round((size - pad * 2) * opticalScale(aspect) * aspect) +
              pad * 2;
        // Radius tracks the slot so a 32px marquee chip isn't rounded like a 56px card tile.
        const radius =
            shape === "round" ? 9999 : Math.min(16, Math.round(size * 0.26));
        ...
        return (
            ...
            >
                <span
                    ...
                    style={{
                        borderRadius: radius,
                        ...(plate ? { background: plate } : undefined),
                    }}
    ...
    return (
        <div
            ...
            className={cn(
                "flex shrink-0 items-center justify-center rounded-2xl bg-brand-gradient font-en text-lg font-extrabold text-white shadow-sm",
                className
            )}
            aria-hidden
        >
...
export function ScoreRing({
    ...
    return (
        ...
        >
            ...
            <span className="absolute font-en text-xs font-extrabold text-brand-900 tabular">
                {value}
            </span>
        </div>
    );
...
export function UniversityCard({
    ...
    return (
        <motion.article
            ...
            className={cn(
                "group relative flex flex-col card p-5 transition-all duration-300 hover:-translate-y-1 hover:shadow-lg hover:border-brand-200",
                selected && "border-brand-400 ring-2 ring-brand-100"
            )}
        >
            <div className="flex items-start gap-4">
                <UniversityLogo
                    name={u.short_name || u.name.en}
                    logo={u.logo_url}
                />
                <div className="min-w-0 flex-1">
                    <h3 className="line-clamp-2 text-base font-bold leading-snug">
                        <Link
                            ...
                            className="after:absolute after:inset-0 focus-ring rounded-lg"
                        >
                            {name}
                        </Link>
                    </h3>
                    {u.district && (
                        <p className="mt-1 flex items-center gap-1 text-xs text-muted">
                            ...
                            {tx(u.district.name, locale)} ·{" "}
                            {t(`common.${u.district.side}`)}
                        </p>
                    )}
                </div>
                <div className="relative z-10 flex shrink-0 items-center gap-1.5">
                    ...
                    >
                        <button
                            type="button"
                            onClick={(e) => e.preventDefault()}
                            aria-label={t(`common.${u.type}`)}
                            className="inline-flex size-7 items-center justify-center rounded-full border border-border text-muted transition-colors hover:border-brand-300 hover:text-brand-800 focus-ring"
                        >
            ...
            <dl className="mt-4 grid grid-cols-2 gap-2 text-center">
                ...
                />
                <Stat
                    label={t("common.programsCount", {
                        count: u.programs.length,
                    })}
                    value={
                        englishCount
                            ? `${formatNumber(englishCount, locale)} EN`
                            : "TR"
                    }
                />
            </dl>
            ...
            <div className="mt-4 flex items-center justify-between border-t border-border pt-3">
                ...
                <button
                    ...
                    className={cn(
                        "relative z-10 inline-flex items-center gap-1.5 rounded-full border px-3 py-1.5 text-xs font-semibold transition-colors focus-ring",
                        selected
                            ? "border-brand-600 bg-brand-600 text-white"
                            : "border-border text-muted hover:border-brand-300 hover:text-brand-800"
                    )}
                ...
                >
                    ...
                    {selected
                        ? t("universities.compareRemove")
                        : t("universities.compareAdd")}
                </button>
            </div>
        </motion.article>
    );
